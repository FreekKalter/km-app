...
kmApp.config([
  ...
  function ($routeProvider, $locationProvider) {
    var d = new Date();
    $routeProvider.when('/input/:date', {
      templateUrl: 'partials/input.html',
      controller: 'kmInput'
    }).when('/overview/:category/:year/:month', {
      templateUrl: 'partials/overview.html',
      controller: 'kmOverviewController'
    }).when('/overview/tijden', { redirectTo: '/overview/tijden/' + d.getFullYear() + '/' + (d.getMonth() + 1) }).when('/overview', { redirectTo: '/overview/kilometers/' + d.getFullYear() + '/' + (d.getMonth() + 1) }).otherwise({ redirectTo: '/input/today' });
  }
]);
'use strict';
var kmControllers = angular.module('kmControllers', []);
kmControllers.controller('kmInput', [
  '$scope',
  '$routeParams',
  '$location',
  '$http',
    ...
    $scope.getState = function (date) {
      $http.get('state/' + date).success(function (data) {
        ...
        if (toFocus !== undefined) {
          setTimeout(function () {
            setFocus(document.getElementById(toFocus));
          }, 100);
        }
      });
    };
...
kmControllers.controller('kmOverviewController', [
  '$scope',
  '$routeParams',
  '$location',
  '$http',
